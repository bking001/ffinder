...
module.exports = function appSocket(socket) {
  ...
  async function setupConnection() {
    ...
    conn.on('ready', () => {
      ...
      login = true;
      socket.emit('status', 'ESTABLISHED');
      socket.emit('statusBackground', '#047857');
      ...
      conn.shell({ term, cols, rows }, (err, stream) => {
        ...
        if(socket.request.session.Devtype == 'BDCOM')
        {
          stream.write('ena\n');
          stream.write(socket.request.session.ena + '\n');   
        }
        else if (socket.request.session.Devtype == 'HUAWEI')
        {
          stream.write('ena' + '\n');
        }
        else if (socket.request.session.Devtype == 'ZTE')
        {
          stream.write('ena\n');
          stream.write(socket.request.session.ena + '\n');   
        }
        else if (socket.request.session.Devtype == 'VSOLUTION')
        {
          //stream.write('ena\n');
          //stream.write(socket.request.session.ena + '\n'); 
        }
        else if (socket.request.session.Devtype == 'CISCO_CATALYST')
        {
          stream.write('ena\n');
          stream.write(socket.request.session.ena + '\n');  
        }
        else if (socket.request.session.Devtype == 'HSGQ')
        {
          //stream.write('ena\n'); 
        }
        ...
        stream.on('data', (data) => {
          ...
          sessionLogs[socket.id] += data;
        });


        const formattedDate = moment().format('YYYY-MM-DD HH:mm:ss'); 
        sessionLogs[`header${socket.id}`] =  `[${formattedDate}] local.INFO: [${socket.request.session.Clientuser}] [${socket.request.session.DevUrl}]\n[From] ${socket.request.session.userip}\n`;

        stream.on('close', (code, signal) => {
          webssh2debug(socket, `STREAM CLOSE: ${util.inspect([code, signal])}`);


          LogData = sessionLogs[`header${socket.id}`] + sessionLogs[socket.id] + '\n'

          sessionLogs[socket.id] = '';      
          sessionLogs[`header${socket.id}`] = '';                

            const logFilePath = `../storage/logs/ssh.log`                   
